...
class Game
  ...
  def start_game
    ...
    x_or_o = ["X", "O"].shuffle
    player_one, player_two = [Computer.new({name: name_one, x_or_o: x_or_o[0], other_mark: x_or_o[1]}), Computer.new({name: name_two, x_or_o: x_or_o[1], other_mark: x_or_o[0]})].shuffle
  ...
  def next_turn
    ...
    position = @current_player.choose_move(@board, winning_lines)
